...
- **asimmetrica** (sbilanciata) a sinistra se $Skewness > 0$ (la media è più grande, .orange[distorta in eccesso])

- **asimmetrica** (sbilanciata) a destra se $Skewness < 0$ (la media è più piccola, .orange[distorta in difetto])
]
.pull-right[
...
.pull-right[
Moda del numero di sigarette fumate al giorno
```{r, echo = FALSE}
epic_sigarette |> 
  slice_max(n) |> 
  kable()
...
I valori di Colesterolo raccolti dai 200 pazienti del database Epic sono (ordinati) i seguenti (espressi in mmol/L):
...
Partendo dai dati così espressi in mmol/L:
$$CV = 1.12/6.074 \times 100 = 18\%$$

e per gli stessi valori espressi in mg/dl abbiamo
$$CV = 43.27/234.43 \times 100 = 18\%$$
...
Il **percentile** $p$-esimo è un valore $V_p$ tale che il $p$\% del campione
assume valori inferiori a esso.

Posto $k$ = "*l'intero più grande inferiore a $np/100$*", il percentile $p$-esimo
è definito come

- la $(k+1)$-ma osservazione (ordinata) se $np/100$ non è un intero.

- la media delle osservazioni $np/100$-ma e $(np/100) + 1$-ma osservazione
(ordinata) se $np/100$ è un intero
...
1. Si calcola $np/100$, pari a:
  - 10-imo percentile (p = 0.1): $10 \times 0.1 = 1$
  
  - 90-esimo percentile (p = 0.9): $10 \times 0.9 = 9$

2. (Sono valori interi, quindi) si calcola la media tra le osservazioni (**ordinate**):
  - 10-imo: posizioni 1 e 2 $\frac{23.3 + 24.9}{2} =$ `r mean(23.3, 24.9)`
  
  - 90-esimo: posizioni 9 e 10 $\frac{41.6 + 42.6}{2} =$ `r mean(41.6, 42.6)`
]
...
1. Si calcola $np/100$, pari a:
  - 20-imo percentile (p = 0.2): $9 \times 0.2 = 1.8$
...
$$s_g^2 = \frac{\sum_{i=1}^k f_i(m_i-\bar{x}_g)^2}{(\sum_{i=1}^k f_i)-1}$$

ovvero
$$s_g^2 = \frac{\sum_{i=1}^k f_im_i^2-n\bar{x}_g^2}{n-1}$$

Per il caso del magnesio è pari a
$$\begin{align*}\bar{s}_g^2& = \frac{[13(106)^2+\ldots+12(352)^2]-100(263.895^2)}{100-1}\\& = 611446.1/99 = 6176.223\end{align*}$$
...
|:----------------:|:---------------------------:|:--------:|:-------------:|:-------
| $[y_1, y_2)$     | $\frac{y_1+y_2}{2}$         | $f_1$    | $y_2-y_1$     | $\frac{f_1}{a_1}$
| $\vdots$         | $\vdots$                    | $\vdots$ | $\vdots$      | $\vdots$
| $[y_k, y_{k+1})$ | $\frac{y_k+y_{k+1}}{2}$     | $f_k$    | $y_{k+1}-y_k$ | $\frac{f_{k+1}}{a_k}$
...
- $H$ è la media tra l'osservazione $(50 / 2)$ª e la $((50 / 2) + 1)$ª, ovvero
  $(215.3+215.4) / 2 = 215.35$
- $h$ è la media dell'osservazione $(50 / 2)$ª e la $((50 / 2) + 1)$ª, ovvero
  $(323.9+324) / 2 = 323.95$
- i limiti per i valori estremi sono quindi
$$x > 323.95 + 1.5 (323.95 - 215.35) = 486.85 x < 52.45$$
- gli outliers
