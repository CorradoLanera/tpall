...
- Il valore predittivo positivo $PV^+$ di un test di screening ? la probabilit? 
che la persona abbia la malattia dato che il test ? positivo
\[
P(\mbox{malattia}|\mbox{test}^+)
\]
- il valore predittivo negativo $PV^-$ di un test di screening ? la probabilit?
che la persona __non__ abbia la malattia dato che il test ? negativo
\[
P(\mbox{no malattia}|\mbox{test}^-)
\]

nel caso della tubercolina si ha che
\[
P(TB|SKT^+)=0.01=PV^+
\]
\[
P(\bar{TB}|SKT^-)= 1-P(TB|SKT^-) =0.9999
\]
...
- La __sensibilit?__ di un test o un sintomo ? la probabilit? di essere positivo al 
test dato che si ? malati 
- la __specificit?__ di un test o un sintomo ? la probabilit? di essere negativo 
al test dato che si ? sani
- si definisce falso negativo la persona che ha un test negativo ma che ? malata 
- si definisce falso positivo la persona che ha un test positivo ma che ? sana

---
# .orange[Test di screening]: esempi


- si supponga che la malattia sia il cancro al polmone e il "test" il fumo di 
sigarette e che la probabilit? di fumare per un soggetto con il cancro al 
polmone pari a 0.90 e a 0.50 per un soggetto sano. Questi due valori sono anche 
la sensibilit? e la specificit? del fumo di sigarette rispetto al cancro 
polmonare 
- si supponga che la malattia sia il cancro al seno nelle donne e che il sintomo 
sia la familiarit? per tumore al seno (madre o sorella con il cancro).
Assumendo che la probabilit? di una donna con cancro al seno di avere 
familiarit? per esso ? pari a 0.05, e che la stessa per le donne senza cancro al 
seno ? pari a 0.02. Si ha che la sensibilit? ? pari allo 0.05, mentre la 
specificit? a $1-0.02=0.98$
...
---
# .orange[Teorema di Bayes]: enunciato base

\small{- Sia $A$ test e $B$ malattia, allora
\[
PV^+=P(B|A) \:\:\: PV^-=P(\bar{B}|\bar{A}) \:\:\: \mbox{SENS}=
P(A|B) \:\:\: \mbox{SPEC}=P(\bar{A}|\bar{B})
\]
- per il teroema di Bayes si ha che
\[
PV^+=Pr(B|A) = \frac{Pr(A|B)Pr(B)}{Pr(A|B)Pr(B)+Pr(A|\bar{B})Pr(\bar{B})}
\]
- $x = Pr(B)$ ? la prevalenza della malattia nella popolazione di 
riferimento
\[
PV^+=\frac{x \cdot SENS}{x \cdot SENS + (1-x) \cdot (1- SPEC)}
\]
\[
PV^-=\frac{(1-x) \cdot SPEC}{(1-x) \cdot SPEC + x \cdot (1- SENS)}
\]}
...
Dalla definizione di probailit? condizionata si ha che
\[
PV^+ = \frac{P(B \cap A)}{P(A)} \:\:\:\:\:\: P(B \cap A) =
P(A|B)P(B)
\]
da teorema delle probabilit? totali si ha che
\[
...
Si supponga che la probabilit? che un iperteso sia classificato tale da una 
macchina per la misurazione automatica della pressione sia pari a 0.84, mentre 
sia pari a 0.23 per un normoteso. Sapendo che la prevalenza dell'ipertensione 
nella popolazione ? pari al 20\%, si vuole calcolare il valore predittivo della 
macchina

Si ha quindi che la sensibilit? = 0.84, la specificit? = 1-0.23 = 0.77 e dal 
teorema di Bayes segue che
\[
PV^+=\frac{0.2(0.84)}{0.2(0.84)+0.8(0.23)}=\frac{0.162}{0.352}=0.48
\]
\[
PV^-=\frac{0.8(0.77)}{0.8(0.77)+0.2(0.16)}=\frac{0.616}{0.648}=0.95
\]
...
Siano $B_1, \ldots, B_k$ un insieme di stati di malattia
mutuamente esclusivi ed esaustivi, nel senso che uno stato di
malattia deve verificarsi necessariamente e non possono
verificarsi due stati allo stesso tempo.

Sia $A$ al presenza di un insieme di sintomi; allora
\[
...
Si supponga che un uomo di et? pari a 60 anni, non fumatore, presenti dei 
sintomi come tosse cronica e affanno occasionale.

Il medico prescrive una biopsia polmonare, il cui risultato ? consistente con 
varie patologie, quali il tumore polmonare, la sarcoidosi, una malattia 
polmonare non fatale, ovvero
\[
\mbox{sintomi}=A=\{\mbox{tosse cronica},\mbox{biopsia}\}
\]
\[
\mbox{stati patologici} = B =
\{\mbox{normale},\mbox{cancro},\mbox{sarcoidosi}\}
\]
sia inoltre \[ P(A|B_1)=0.001 \:\:\: P(A|B_2)=0.9 \:\:\:
P(A|B_3)=0.9
\]
\[
...
Si vuole conoscere le probabilit? degli stati patologici dati i sintomi
\begin{eqnarray*}
...
\[
P(B_2|A)=
\frac{0.9(0.001)}{0.001(0.99)+0.9(0.001)+0.9(0.009)}=0.090
\]
\[
P(B_3|A)=
...
\]
la probabilit? degli stati patologici ?
\[
P(B_1|A)=
