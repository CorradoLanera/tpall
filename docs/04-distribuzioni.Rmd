...
Una funzione di massa di probabilità è una relazione matematica, o regola, che assegna **ad ogni possibile valore** $r$ di una variabile casuale discreta $X$ **la (sua) probabilità** $P(X=r)$.
...
\small Per la variabile $X$=\em{pazienti complianti al controllo dell'ipertensione}
```{r, include=FALSE}
MEAN0=0*(0.008)+1*(0.076)+2*(0.265)+3*(0.411)+4*(0.240)
```
\[
E(X)=0(0.008)+1(0.076)+2(0.265)+3(0.411)+4(0.240) = `r MEAN0`
\]
...
\scriptsize Sia $X$ il numero di episodi di otiti nei primi due anni di vita e la rispettiva massa di probabilità sia:
...
```{r, include=FALSE}
MEAN=0*(0.129)+1*(0.264)+2*(0.271)+3*(0.185)+4*(0.095)+5*(0.039)+6*(0.017)
```

il numero atteso di episodi di episodi di otite nei primi due anni di vita è pari a
\vspace{0.5cm}
\[
E(X) = 0(0.129)+1(0.264)+2(0.271)+3(0.185)+4(0.095)+5(0.039)+6(0.017)= `r MEAN`
\]
...
vale anche
\[
\sigma^2=E(X-\mu)=\sum_{i=1}^kx_i^2P(X=x_i)-\mu^2
\]

```{r, include=FALSE}
var=(0^2*(0.129)+1^2*(0.264)+2^2*(0.271)+3^2*(0.185)+4^2*(0.095)+5^2*(0.039)+6^2*(0.017))-MEAN^2
```
\small
La varianza della variabile casuale episodi di otite è pari a
\[
0^2(0.129)+\ldots+36(0.017)-2.038^2=`r round(var,3)`
\]
